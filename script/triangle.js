...
function setVariables() {
	...
	colors = [["28","254","255"],["120","204","204"],["10","155","155"]];
}
...
function draw( start ) {
	...
	var lengte = Math.floor(Math.random() * 100);
...
function rotatePoints( point, hoek, start ) {
	return p = {
	X: start.X + Math.cos( hoek ) * (point.X - start.X) - Math.sin( hoek ) * (point.Y - start.Y),
	Y: start.Y + Math.sin( hoek ) * (point.X - start.X) + Math.cos( hoek ) * (point.Y - start.Y)
	}
}
